      ...
      scope :annual_counts, -> () {
        ...
        scope :csv, -> (options = {}) {
          CSV.generate(options) do |csv|
            ...
            fields = Field.select("publications.id, GROUP_CONCAT(description, '; ') AS description").joins(:publications).group("publications.id").reduce({}) { |result, a| result.update(a.id => a.description) }
            focusgroups = Focusgroup.select("publications.id, GROUP_CONCAT(description, '; ') AS description").joins(:publications).group("publications.id").reduce({}) { |result, a| result.update(a.id => a.description) }
            platforms = Platform.select("publications.id, GROUP_CONCAT(description, '; ') AS description").joins(:publications).group("publications.id").reduce({}) { |result, a| result.update(a.id => a.description) }
            locations = Location.select("publications.id, GROUP_CONCAT(description, '; ') AS description").joins(:publications).group("publications.id").reduce({}) { |result, a| result.update(a.id => a.description) }
